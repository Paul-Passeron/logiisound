...
Resistor::Resistor(double r, int n1, int n2)
    : resistance(r), node1(n1), node2(n2) {
  if (r <= 0) {
    throw std::runtime_error(
        "Can't have resistor with resistance <= 0. Replace with a conneXdon.");
  }
}

void Resistor::stamp(Eigen::MatrixXd &matrix, Eigen::VectorXd &rhs, double currentTime, double dt) {
